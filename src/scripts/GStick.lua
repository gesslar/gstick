...
function GStick:UpdateFoe()
  ...
  if status == "mortally wounded" then
    health = 0
  elseif status == "nearly dead" then
    health = 4
  elseif status == "in very bad shape" then
    health = 10
  elseif status == "in bad shape" then
    health = 20
  elseif status == "not in good shape" then
    health = 50
  elseif status == "slightly hurt" then
    health = 95
  elseif status == "in good shape" then
    health = 100
  else
    status = "unknown"
    health = 100
...
GStick.EventHandlers.UpdateVitals = registerNamedEventHandler(GStick.appName, "GStickUpdateVitals", "gmcp.Char.Vitals",
  "GStick:UpdateVitals")
GStick.EventHandlers.UpdateXP = registerNamedEventHandler(GStick.appName, "GStickUpdateXP", "gmcp.Char.Status",
  "GStick:UpdateXP")
GStick.EventHandlers.UpdateFoe = registerNamedEventHandler(GStick.appName, "GStickUpdateFoe", "gmcp.Char.Status",
  "GStick:UpdateFoe")
